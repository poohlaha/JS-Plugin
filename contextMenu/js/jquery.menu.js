            ...
            _showMenu : function(_left,_top){
                ...
                if(this.option.data && this.option.data.length > 0){
                    ...
                    for(var i = 0,len = this.option.data.length;i < len;i++){
                        ...
                        var _menu_body_img = $('<img src="'+(_data.img.src || this.defaultImageSrc)+'" width="'+(_data.img.width || this.defaultImageWidth)+'" class="'+(_data.img.cls || '')+'" height="'+(_data.img.height || this.defaultImageHeight)+'" style="float: left">');
                        var _menu_body_a = '';
                        var _func = _data.click;
                        if(!_func || typeof _func !== "function")
                            _menu_body_a = $('<a href="javascript:;" class="'+(this.option.menu_li_a_cls)+'">'+(_data.text || '')+'</a>');
                        else
                            _menu_body_a = $('<a href="javascript:;" class="'+(this.option.menu_li_a_cls)+'">'+(_data.text || '')+'</a>');

                        var _menu_body_li = $('<li class="'+this.option.menu_li_cls+'" tab-index = "'+(i + 1 + "_" + this._key)+'"></li>');
                        //_menu_body_a.click(this._executeFun.call(this,_func));
                        _map.put( i + 1 +"_" + this._key,_func);
                        _menu_body_li.append(_menu_body_img);
                        _menu_body_li.append(_menu_body_a);
                ...
                if(!_map.isEmpty()){
                    var self = this;
                    _menu_box.find('li').each(function(){
                        if($(this).hasClass(self.option.menu_li_cls)){
                            if($(this).attr('tab-index')){
                                var func = _map.get($(this).attr('tab-index'));
                                if(func){
